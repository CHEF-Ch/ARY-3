...
5. 将 `origin/B_parts` 的 B race-mgmt 加固提交合入 `main`，合并后复跑 `server` 的 `npm.cmd run test:race-mgmt` 与 `client` 的 `npm.cmd run build`。
6. 在有 Git Bash 的环境中复跑 `server/src/modules/portfolio/test.sh`，确认 shell 脚本入口与已通过的 Node 等价验证一致。
