...
* `PRD-TEMP-1` v0.4 一致性整改已完成并待复审：核心对象、CAConnection 登记 / Session 接收窗口、作品内容准入、ReviewFlag 和疑似违规人工处理规则已统一。
...
| `PRD-TEMP-1` 报名 / RaceProject / CA 参赛语义整改 | 待复审 | v0.4 一致性整改完成：旧锁定事件和 RaceProject 旧关系已清理，窗口规则已落到实现，race-mgmt / portfolio 回归通过。 | `docs/registration-ca-rules-alignment.taskbook.md`、`server/src/modules/race-mgmt/security-smoke-test.mjs`、`server/src/modules/portfolio/test.sh` |
| `UX-1` UX/UI 高保真原型与设计基线 | 进行中 | 高保真原型已按 IA 重构。 | `design-prototype/index.html` |
| `B` race-mgmt 实现 | 已完成 | 已合入 `main`；本轮补齐握手失败和连接禁用审计事件，`npm.cmd run test:race-mgmt` 通过。 | `server/src/modules/race-mgmt/` |
| `C` portfolio 实现 | 已完成 | 后端 + 前端四个页面已完成；本轮补齐脱敏审计摘要及 failed / not_configured 非门禁回归，Git Bash 测试和 server/client build 通过。 | `server/src/modules/portfolio/`、`client/src/pages/works/WorksPage.tsx`、`client/src/pages/results/ResultsPage.tsx`、`client/src/pages/console/JudgeView.tsx`、`client/src/pages/console/OrganizerJudging.tsx` |
