...
| `B` race-mgmt 实现 | 待复审 | 已完成第一至四阶段加固及动态传输安全复查：Race 状态流转按主流程限制；Race 级管理权限不再因缺少 race scope 默认放行；`npm run test:race-mgmt` smoke test 覆盖关键安全与幂等路径；Race Page、Organizer Overview、Rider View 已去除开发验收口吻；主办方可在 UI 推进赛事状态；选手可用一次性凭据完成 DCR 握手并禁用异常连接；Organizer / Rider 均可查看 CA 接收审计摘要和最近拒收原因；本轮修复拒收 nonce 燃烧、payload 大小限制和审计响应脱敏问题。 | `server/src/modules/race-mgmt/routes.ts`、`server/src/app.ts`、`server/src/shared/auth.ts`、`server/src/modules/race-mgmt/security-smoke-test.mjs`、`client/src/pages/race/RacePage.tsx`、`client/src/pages/console/OrganizerOverview.tsx`、`client/src/pages/console/RiderView.tsx` |
